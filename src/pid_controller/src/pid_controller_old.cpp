...
public:
  ...
  bool stopped = 0;
  double scale_speed = 16384/(15.63*odo_perimeter);//电机的扭矩电流 -16384~0~16384 对应 -20A~0~20A
...
void PidController::updateOdometry(int steer_angular, int motor_angular, int round){
...
int main(int argc, char** argv)
{
  ...
  while(ros::ok()){
      ...
      if(ser.available()){
            ...
            if(!isFirst){
                pid_controller.updateOdometry(odometry_steer,odometry_motor,odometry_round);
            }
            isFirst = 0;
      }
  //ros::spin();
