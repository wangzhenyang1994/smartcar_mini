...
class TrafficLightClassifier():    
    ...
    def __init__(self, pca_path, clf_path, image_dims=(20, 60), image_channels=3):
        ...
        pcapath = os.path.join(currentpath, pca_path)
        clfpath = os.path.join(currentpath, clf_path)
...
class FrameProcessor():
    ...
    def has_red_lights(self):
        return True if self.red_lights else False
...
class trafficLightDetector:
    ...
    def callback(self, imgmsg):
        ...
        result = self.frame_processor.traffic_lights_in_frame(img)
        self.has_red_light.data = self.frame_processor.has_red_lights()
        self.pub.publish(self.has_red_light)
        cv2.imshow("tracffic_light", result)
        cv2.waitKey(1)
