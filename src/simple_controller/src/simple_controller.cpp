...
*/
  ...
  {//buffer[8]
      ...
      if(temp.length() > 2){
          temp = temp.substr(temp.size() - 2);//last two number
      }
      uint8_t result;
...
void SimpleController::cmdCallback(const geometry_msgs::Twist::ConstPtr & cmd){
    ...
    int ratio = 10000;//电机的扭矩电流 -16384~0~16384 对应 -20A~0~20A
    int speed = 0;
    int speed_l, speed_h;
...
/**
    ...
    speed = (△motor_round/total_ratio+△round)*后轮周长
    
*/

void SimpleController::updateOdometry(int steer_angular, int motor_round, int round, int rate){
...
int main(int argc, char** argv)
{
  ...
  while(ros::ok()){
      ...
      if(ser.available()){
            ...
            if(!isFirst){
                simple_controller.updateOdometry(odometry_steer,odometry_motor,odometry_round,rate);
            }
            isFirst = 0;
